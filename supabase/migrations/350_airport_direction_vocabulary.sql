-- ============================================================================
-- 350 — airport directions come from the tenant's vocabulary
-- ============================================================================
-- airport_staff_rates.direction is arrival | departure | both, and the
-- pricing engine selects on it: an arrival day asks for 'arrival' or 'both',
-- a departure day for 'departure' or 'both'. Those KEYS stay exactly as they
-- are; this only lets the agency choose the words shown for them ("Arrivée")
-- through Settings → Your vocabulary like every other rate-form dropdown.
-- An entry the agency adds is offered on the form and stored on the rate,
-- and the engine simply never selects it — the settings description says so.
--
-- Rows already store the keys; nothing is re-filed.
--
-- seed_tenant_vocabulary is redefined in full (349's blocks verbatim plus
...
ALTER TABLE tenant_vocabularies ADD CONSTRAINT tenant_vocabularies_kind_check
  CHECK (kind IN (
    ...
    'guide_duration', 'guide_language', 'rate_season', 'airline', 'hotel_supplement', 'airport_direction'
  ));
...
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'airport_direction');
  END LOOP;
END $$;
...
-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT count(*) FROM tenant_vocabularies WHERE kind = 'airport_direction';  -- 3 × tenants
