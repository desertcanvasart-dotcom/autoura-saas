-- ============================================================================
-- 347 — the season a rate row is tagged with comes from the vocabulary
-- ============================================================================
-- Two things are called "season" in this app and they stay separate:
--
--   * The DEMAND CALENDAR (Settings → Seasonal Premiums, migration 304):
--     the operator's own seasons, with dates and an uplift on the selling
--     price. The engine reads it. Untouched here.
--   * The SEASON TAG on a rate row (entrance_fees.season,
--     sleeping_train_rates.season, and the same column on flight, meal,
--     activity and guide rates): the supplier's season a fare belongs to —
--     "peak season sleeper fare". A label the agency files rates under; the
--     engine does not read it.
--
-- The tag's dropdown was hardcoded twice with two different lists (the
-- attraction form offered keys, the sleeper form offered labels). It is now
-- one vocabulary kind, rate_season, and both forms read it. Rows that stored
-- a label are re-filed under the key; "Year Round" and "All Year" both mean
-- all_year.
--
-- seed_tenant_vocabulary is redefined in full (346's blocks verbatim plus
...
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'rate_season');
  END LOOP;
END $$;

-- Re-file label-stored season tags under their keys (345's transform).
...
UPDATE entrance_fees
   SET season = CASE lower(btrim(season)) WHEN 'year round' THEN 'all_year' WHEN 'all year' THEN 'all_year' ELSE pg_temp.vocab_slug(season) END
 WHERE season IS NOT NULL AND season ~ '[^a-z0-9_]';
UPDATE sleeping_train_rates
   SET season = CASE lower(btrim(season)) WHEN 'year round' THEN 'all_year' WHEN 'all year' THEN 'all_year' ELSE pg_temp.vocab_slug(season) END
 WHERE season IS NOT NULL AND season ~ '[^a-z0-9_]';
UPDATE flight_rates
   SET season = CASE lower(btrim(season)) WHEN 'year round' THEN 'all_year' WHEN 'all year' THEN 'all_year' ELSE pg_temp.vocab_slug(season) END
...
-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT count(*) FROM tenant_vocabularies WHERE kind = 'rate_season';  -- 6 × tenants
--   SELECT count(*) FROM sleeping_train_rates WHERE season ~ '[^a-z0-9_]';  -- 0
