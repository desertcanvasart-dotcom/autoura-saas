-- ============================================================================
-- 346 — guide languages come from the tenant's vocabulary
-- ============================================================================
-- The guide rate form's language list was the last hardcoded dropdown after
-- 345. It stayed behind because the pricing engine matches a quote's
-- language STRING ("English") against guide_rates.guide_language. The
-- engine now resolves that string through the tenant's guide_language
-- vocabulary first (label, key, or slug of either) and only then queries,
-- so rows store the KEY like every other vocabulary column and the agency
-- may call English whatever it likes — or add Mandarin.
--
-- Existing rows stored the label; they are re-filed under the key with the
-- slugifyKey transform ('English' → english). Quotes, itineraries and tour
-- variations keep their language text untouched: that is what the engine
-- resolves FROM, and what messaging and AI prompts read.
--
-- seed_tenant_vocabulary is redefined in full (345's blocks verbatim plus
...
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'guide_language');
  END LOOP;
END $$;

-- Re-file label-stored languages under their keys (345's transform).
...
UPDATE guide_rates SET guide_language = pg_temp.vocab_slug(guide_language) WHERE guide_language ~ '[^a-z0-9_]';

COMMIT;

-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT count(*) FROM tenant_vocabularies WHERE kind = 'guide_language';  -- 12 × tenants
--   SELECT count(*) FROM guide_rates WHERE guide_language ~ '[^a-z0-9_]';      -- 0
