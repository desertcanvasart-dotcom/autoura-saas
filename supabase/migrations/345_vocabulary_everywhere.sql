-- ============================================================================
-- 345 — every rate-form dropdown comes from the tenant's vocabulary
-- ============================================================================
-- The Add Attraction, Tipping, Transportation, Flight, Airport/Hotel service,
-- Meal, Activity and Guide forms each carried their own hardcoded list —
-- attraction categories, tipping roles and contexts, cuisines, activity
-- types, guide grades and more — that no agency could touch. Twenty of those
-- lists become vocabulary kinds here, alongside the nine from 334/341, so
-- Settings → Your vocabulary is the one place every dropdown reads from.
--
-- KEYS ARE STABLE; LABELS ARE THE AGENCY'S. Where the pricing engine selects
-- on a value (transport 'airport_transfer'/'day_tour'/'half_day', guide
-- 'egyptologist'/'senior' and 'full_day'/'half_day'/'meet_greet', tipping
-- 'per_day', airport 'meet_greet' and hotel 'checkin_assist'/'porter'
-- fallbacks, flight 'economy') the KEY keeps working whatever the label
-- says; an entry the agency adds is offered in the form and stored, and the
-- engine simply does not select on it. transport_service_type carries
-- meta.needs_destination (the form asks for a destination city when true),
-- the way vehicle_type carries passenger bands. guide_grade seeds the six
-- legacy grades HIDDEN so rows filed under them still read as words.
--
-- Rows that stored the LABEL (meals: cuisine / restaurant type / dietary
-- options; activities: category / type / duration / unit) are re-filed under
-- the key, the slugifyKey transform ('Middle Eastern' → middle_eastern,
-- 'Café' → cafe). Everything else already stored keys.
--
-- seed_tenant_vocabulary is redefined in full (341's blocks verbatim, then
-- the new ones): the preset lives in one place, and this is now that place.
--
...
-- Every existing tenant gets the twenty new lists (gaps only).
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'attraction_category');
    PERFORM seed_tenant_vocabulary(r.id, 'attraction_fee_type');
    PERFORM seed_tenant_vocabulary(r.id, 'tipping_role');
    PERFORM seed_tenant_vocabulary(r.id, 'tipping_context');
    PERFORM seed_tenant_vocabulary(r.id, 'tipping_unit');
    PERFORM seed_tenant_vocabulary(r.id, 'transport_service_type');
    PERFORM seed_tenant_vocabulary(r.id, 'flight_type');
    PERFORM seed_tenant_vocabulary(r.id, 'flight_cabin');
    PERFORM seed_tenant_vocabulary(r.id, 'flight_frequency');
    PERFORM seed_tenant_vocabulary(r.id, 'airport_service_type');
    PERFORM seed_tenant_vocabulary(r.id, 'hotel_service_type');
    PERFORM seed_tenant_vocabulary(r.id, 'cuisine_type');
    PERFORM seed_tenant_vocabulary(r.id, 'restaurant_type');
    PERFORM seed_tenant_vocabulary(r.id, 'dietary_option');
    PERFORM seed_tenant_vocabulary(r.id, 'activity_category');
    PERFORM seed_tenant_vocabulary(r.id, 'activity_type');
    PERFORM seed_tenant_vocabulary(r.id, 'activity_duration');
    PERFORM seed_tenant_vocabulary(r.id, 'activity_unit');
    PERFORM seed_tenant_vocabulary(r.id, 'guide_grade');
    PERFORM seed_tenant_vocabulary(r.id, 'guide_duration');
  END LOOP;
END $$;

-- ----------------------------------------------------------------------------
-- Re-file label-stored columns under their keys. Same transform as
-- lib/vocabulary slugifyKey: strip accents, lowercase, runs of
-- non-alphanumerics → one underscore, trimmed. Only rows still holding a
-- label (anything outside [a-z0-9_]) are touched, so re-running is a no-op.
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION pg_temp.vocab_slug(t TEXT) RETURNS TEXT AS $$
  SELECT btrim(regexp_replace(lower(btrim(translate(t,
    'ÁÀÂÄÃÅáàâäãåÉÈÊËéèêëÍÌÎÏíìîïÓÒÔÖÕóòôöõÚÙÛÜúùûüÇçÑñ',
    'AAAAAAaaaaaaEEEEeeeeIIIIiiiiOOOOOoooooUUUUuuuuCcNn'))), '[^a-z0-9]+', '_', 'g'), '_')
$$ LANGUAGE sql IMMUTABLE;

UPDATE meal_rates SET cuisine_type    = pg_temp.vocab_slug(cuisine_type)    WHERE cuisine_type    ~ '[^a-z0-9_]';
UPDATE meal_rates SET restaurant_type = pg_temp.vocab_slug(restaurant_type) WHERE restaurant_type ~ '[^a-z0-9_]';
UPDATE meal_rates
   SET dietary_options = (SELECT jsonb_agg(pg_temp.vocab_slug(e)) FROM jsonb_array_elements_text(dietary_options) e)
 WHERE jsonb_typeof(dietary_options) = 'array'
   AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(dietary_options) e WHERE e ~ '[^a-z0-9_]');

UPDATE activity_rates SET activity_category = pg_temp.vocab_slug(activity_category) WHERE activity_category ~ '[^a-z0-9_]';
UPDATE activity_rates SET activity_type     = pg_temp.vocab_slug(activity_type)     WHERE activity_type     ~ '[^a-z0-9_]';
UPDATE activity_rates SET duration          = pg_temp.vocab_slug(duration)          WHERE duration          ~ '[^a-z0-9_]';
UPDATE activity_rates SET unit_label        = pg_temp.vocab_slug(unit_label)        WHERE unit_label        ~ '[^a-z0-9_]';

COMMIT;

-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT kind, count(*) FROM tenant_vocabularies GROUP BY 1 ORDER BY 1;   -- 29 kinds × tenants
--   SELECT count(*) FROM meal_rates     WHERE cuisine_type ~ '[^a-z0-9_]' OR restaurant_type ~ '[^a-z0-9_]';  -- 0
--   SELECT count(*) FROM activity_rates WHERE activity_category ~ '[^a-z0-9_]' OR duration ~ '[^a-z0-9_]';   -- 0
