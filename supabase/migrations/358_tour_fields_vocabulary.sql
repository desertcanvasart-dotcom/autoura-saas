-- ============================================================================
-- 358 — the four tour fields become the tenant's own words
-- ============================================================================
-- tour_type, physical_level, best_for and the tour theme were the last
-- dropdowns in the app an agency could not change. The first three were
-- hardcoded arrays in TourManagerContent.tsx; the theme came from
-- tour_categories, a table nothing seeded, no screen could add to, and which
-- was EMPTY for every tenant — a dropdown with no options, and a UUID foreign
-- key hanging off it. All four are vocabulary now.
--
-- KEYS STAY, WORDS CHANGE — the 351 rule:
--   tour_type keeps day_tour / multi_day / stopover, because two of those keys
--   change behaviour: day_tour and stopover are measured in HOURS and held to
--   a single day. An entry an agency ADDS behaves like a multi-day tour, and
--   the settings description says so.
--   physical_level keeps easy / moderate / challenging / demanding. Rows
--   already store those keys; nothing is re-filed.
--
-- best_for IS re-filed, because it is the one field that stored LABELS, not
-- keys: arrays like {Families,"Solo Travelers"}. Vocabulary rows store keys,
-- so the existing arrays are slugified in place below. Measured first: one
-- template in the whole database has a non-empty best_for.
--
-- tour_theme is a NEW column. Nothing is migrated into it because
-- tour_categories held nothing to migrate.
--
-- This migration is ADDITIVE and safe to apply before the deploy. Retiring
-- tour_categories itself is 359, which must wait until the code that joins it
-- has stopped running (the 240 rule).
--
-- seed_tenant_vocabulary is redefined in full (351's blocks verbatim plus the
-- four new ones): the preset lives in one place, and this is now that place.
--
...
-- The theme a tour is filed under, as a vocabulary KEY — the same shape as
-- every other vocabulary field on a row. Replaces tour_templates.category_id,
-- a UUID pointing at a table that was empty everywhere.
ALTER TABLE tour_templates ADD COLUMN IF NOT EXISTS tour_theme VARCHAR(50);

COMMENT ON COLUMN tour_templates.tour_theme IS
  'tenant_vocabularies key of kind tour_theme. Replaced category_id (359).';
...
BEGIN
  ...
  IF p_kind IS NULL OR p_kind = 'tour_type' THEN
    INSERT INTO tenant_vocabularies (tenant_id, kind, key, label, description, rank) VALUES
      (p_tenant, 'tour_type', 'day_tour',  'Day Tour',       'One day, measured in hours',        1),
      (p_tenant, 'tour_type', 'multi_day', 'Multi-Day Tour', 'Several days, with nights',         2),
      (p_tenant, 'tour_type', 'stopover',  'Stopover Tour',  'A layover trip, measured in hours', 3)
    ON CONFLICT ON CONSTRAINT tenant_vocabularies_unique_key DO NOTHING;
    GET DIAGNOSTICS n = ROW_COUNT; inserted := inserted + n;
  END IF;

  IF p_kind IS NULL OR p_kind = 'tour_physical_level' THEN
    INSERT INTO tenant_vocabularies (tenant_id, kind, key, label, description, rank) VALUES
      (p_tenant, 'tour_physical_level', 'easy',        'Easy',        'Suitable for all',      1),
      (p_tenant, 'tour_physical_level', 'moderate',    'Moderate',    'Some walking',          2),
      (p_tenant, 'tour_physical_level', 'challenging', 'Challenging', 'Active travellers',     3),
      (p_tenant, 'tour_physical_level', 'demanding',   'Demanding',   'Fit travellers only',   4)
    ON CONFLICT ON CONSTRAINT tenant_vocabularies_unique_key DO NOTHING;
    GET DIAGNOSTICS n = ROW_COUNT; inserted := inserted + n;
  END IF;

  IF p_kind IS NULL OR p_kind = 'tour_best_for' THEN
    INSERT INTO tenant_vocabularies (tenant_id, kind, key, label, description, rank) VALUES
      (p_tenant, 'tour_best_for', 'families',            'Families',            NULL,  1),
      (p_tenant, 'tour_best_for', 'couples',             'Couples',             NULL,  2),
      (p_tenant, 'tour_best_for', 'solo_travelers',      'Solo Travelers',      NULL,  3),
      (p_tenant, 'tour_best_for', 'groups',              'Groups',              NULL,  4),
      (p_tenant, 'tour_best_for', 'seniors',             'Seniors',             NULL,  5),
      (p_tenant, 'tour_best_for', 'history_buffs',       'History Buffs',       NULL,  6),
      (p_tenant, 'tour_best_for', 'adventure_seekers',   'Adventure Seekers',   NULL,  7),
      (p_tenant, 'tour_best_for', 'photography',         'Photography',         NULL,  8),
      (p_tenant, 'tour_best_for', 'relaxation',          'Relaxation',          NULL,  9),
      (p_tenant, 'tour_best_for', 'first_time_visitors', 'First-time Visitors', NULL, 10),
      (p_tenant, 'tour_best_for', 'repeat_visitors',     'Repeat Visitors',     NULL, 11),
      (p_tenant, 'tour_best_for', 'luxury_travelers',    'Luxury Travelers',    NULL, 12),
      (p_tenant, 'tour_best_for', 'budget_travelers',    'Budget Travelers',    NULL, 13)
    ON CONFLICT ON CONSTRAINT tenant_vocabularies_unique_key DO NOTHING;
    GET DIAGNOSTICS n = ROW_COUNT; inserted := inserted + n;
  END IF;

  IF p_kind IS NULL OR p_kind = 'tour_theme' THEN
    INSERT INTO tenant_vocabularies (tenant_id, kind, key, label, description, rank) VALUES
      (p_tenant, 'tour_theme', 'cultural',   'Cultural',            'History, museums and monuments', 1),
      (p_tenant, 'tour_theme', 'adventure',  'Adventure',           'Desert, diving, active travel',  2),
      (p_tenant, 'tour_theme', 'relaxation', 'Beach & Relaxation',  'Resorts and slow days',          3),
      (p_tenant, 'tour_theme', 'religious',  'Religious',           'Pilgrimage and holy sites',      4),
      (p_tenant, 'tour_theme', 'cruise',     'Cruise',              'Nile and lake sailings',         5),
      (p_tenant, 'tour_theme', 'honeymoon',  'Honeymoon',           'Couples and special occasions',  6)
    ON CONFLICT ON CONSTRAINT tenant_vocabularies_unique_key DO NOTHING;
    GET DIAGNOSTICS n = ROW_COUNT; inserted := inserted + n;
  END IF;
...
-- Existing tenants get the four new lists; ON CONFLICT DO NOTHING inside the
-- function means an agency that has already renamed something keeps it.
DO $$
DECLARE
  r RECORD;
  k TEXT;
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    FOREACH k IN ARRAY ARRAY['tour_type', 'tour_physical_level', 'tour_best_for', 'tour_theme']
    LOOP
      PERFORM seed_tenant_vocabulary(r.id, k);
    END LOOP;
  END LOOP;
END $$;

-- best_for stored the LABEL an agency clicked ('Solo Travelers'), not a key.
-- Vocabulary rows are keyed, so re-file the arrays: 'Solo Travelers' becomes
-- 'solo_travelers', matching the seeded keys above. Order is preserved, and a
-- value already in key form is left exactly as it is.
UPDATE tour_templates
   SET best_for = (
         SELECT array_agg(
                  trim(both '_' from lower(regexp_replace(elem, '[^a-zA-Z0-9]+', '_', 'g')))
                  ORDER BY ord
                )
           FROM unnest(best_for) WITH ORDINALITY AS t(elem, ord)
       )
 WHERE best_for IS NOT NULL
   AND array_length(best_for, 1) > 0
   AND EXISTS (
         SELECT 1 FROM unnest(best_for) AS e
          WHERE e <> trim(both '_' from lower(regexp_replace(e, '[^a-zA-Z0-9]+', '_', 'g')))
       );
...
-- Every tenant got all four lists. A tenant left without them would meet four
-- empty dropdowns, which is the state this migration exists to end.
DO $$
DECLARE
  k TEXT;
  missing INTEGER;
    ...
    SELECT count(*) INTO missing
     ...
     WHERE NOT EXISTS (
       SELECT 1 FROM tenant_vocabularies v
        WHERE v.tenant_id = t.id AND v.kind = k
     );
    IF missing > 0 THEN
      RAISE EXCEPTION '% tenant(s) have no % vocabulary', missing, k;
    END IF;
  END LOOP;
END $$;

-- The three behaviour-bearing tour_type keys must exist for every tenant:
-- day_tour and stopover are what the app measures in hours instead of days.
DO $$
DECLARE
  bad INTEGER;
BEGIN
  SELECT count(*) INTO bad
    FROM tenants t
   WHERE (SELECT count(*) FROM tenant_vocabularies v
           WHERE v.tenant_id = t.id AND v.kind = 'tour_type'
             AND v.key IN ('day_tour', 'multi_day', 'stopover')) < 3;
  IF bad > 0 THEN
    RAISE EXCEPTION '% tenant(s) are missing a built-in tour_type key', bad;
  END IF;
END $$;

-- No best_for entry may still be in label form, or the form would show a tag
-- the agency cannot see in their vocabulary list.
DO $$
DECLARE
  bad TEXT;
BEGIN
  SELECT e INTO bad
    FROM tour_templates, unnest(best_for) AS e
   WHERE e <> trim(both '_' from lower(regexp_replace(e, '[^a-zA-Z0-9]+', '_', 'g')))
   LIMIT 1;
  IF bad IS NOT NULL THEN
    RAISE EXCEPTION 'best_for still holds a label, not a key: %', bad;
  END IF;
END $$;

-- The new column exists and nothing has been dropped yet: 359 retires
-- tour_categories, and only after the deploy that stops joining it.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'tour_templates'
       AND column_name = 'tour_theme'
  ) THEN
    RAISE EXCEPTION 'tour_templates.tour_theme was not created';
  END IF;
END $$;
