-- ============================================================================
-- 380 — guide modes: Spot / Throughout are the agency's words, and a guide
--       rate says which mode it is for (sibling #464)
-- ============================================================================
-- A quote's guide is either a SPOT guide (a local guide on each sightseeing
-- day) or a THROUGHOUT guide (one guide travels the whole trip). The
-- calculator has offered both since migration 326 — but the rate form could
-- not say which mode a rate is for, so a Throughout quote was priced from the
-- Spot rates. Operator, 2026-09-17: "in the guide vocabulary section we don't
-- have the option to add spot and throughout; same for the guide form."
--
--   1. A vocabulary kind `guide_mode`, seeded Spot / Throughout for every
--      tenant (and for new ones, through seed_tenant_vocabulary), so the
--      calculator's buttons carry the agency's own words.
--   2. guide_rates.guide_mode — the mode a rate is for. Every existing row is
--      a Spot rate: that is what they have always been priced as.
--
-- seed_tenant_vocabulary is redefined in full (358's blocks verbatim plus
-- this one): the preset lives in one place, and this is now that place.
-- Its EXECUTE stays revoked from the browser roles (361).
--
...
BEGIN
  ...
  IF p_kind IS NULL OR p_kind = 'guide_mode' THEN
    INSERT INTO tenant_vocabularies (tenant_id, kind, key, label, description, rank) VALUES
      (p_tenant, 'guide_mode', 'spot',       'Spot guide',       'A local guide on each sightseeing day, in that city',                      1),
      (p_tenant, 'guide_mode', 'throughout', 'Throughout guide', 'One guide travels with the group the whole trip (fee every day, a bed each night, one vehicle seat)', 2)
    ON CONFLICT ON CONSTRAINT tenant_vocabularies_unique_key DO NOTHING;
    GET DIAGNOSTICS n = ROW_COUNT; inserted := inserted + n;
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.seed_tenant_vocabulary(uuid, text) FROM PUBLIC, anon, authenticated;

-- Existing tenants get the two modes; ON CONFLICT DO NOTHING inside the
-- function means an agency that has already renamed one keeps it.
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'guide_mode');
  END LOOP;
END $$;

-- The mode a guide rate is for. A vocabulary KEY (an agency may add a third
-- mode); every row so far is a Spot rate.
ALTER TABLE guide_rates ADD COLUMN IF NOT EXISTS guide_mode VARCHAR(50) NOT NULL DEFAULT 'spot';
ALTER TABLE guide_rates DROP CONSTRAINT IF EXISTS guide_rates_guide_mode_key_shape;
ALTER TABLE guide_rates ADD CONSTRAINT guide_rates_guide_mode_key_shape CHECK (guide_mode ~ '^[a-z0-9_]{1,50}$');
COMMENT ON COLUMN guide_rates.guide_mode IS
  'The guide mode this rate is for: a key of the agency''s guide_mode vocabulary (spot = a local guide per sightseeing day; throughout = one guide for the whole trip). Pricing reads only rates of the quote''s mode.';
