-- ============================================================================
-- 348 — airlines come from the tenant's vocabulary
-- ============================================================================
-- The flight rate form and its API each carried a hardcoded list of sixteen
-- carriers with IATA codes. An agency selling on Air Sinai or a charter
-- carrier had "Other" and a typed name. The list is now a vocabulary kind,
-- airline, with the IATA code in meta.code — edited in Settings → Your
-- vocabulary like a vehicle's passenger band.
--
-- flight_rates.airline stored the carrier NAME and is read everywhere a
-- flight is named (engine service lines and holes, the tour manager's
-- ticket picker, the pricing grid, the AI catalogue). It now stores the KEY
-- like every other vocabulary column; each of those readers shows the
-- agency's word. Existing rows are re-filed under the key with the slugifyKey
-- transform ('EgyptAir' → egyptair). A carrier that came in through a
-- supplier and is not in the list keeps a slug of its name and still reads
-- as that slug until the agency adds it.
--
-- seed_tenant_vocabulary is redefined in full (347's blocks verbatim plus
...
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'airline');
  END LOOP;
END $$;

-- Re-file label-stored airlines under their keys (345's transform).
...
UPDATE flight_rates SET airline = pg_temp.vocab_slug(airline) WHERE airline ~ '[^a-z0-9_]';

COMMIT;

-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT count(*) FROM tenant_vocabularies WHERE kind = 'airline';  -- 17 × tenants
--   SELECT count(*) FROM flight_rates WHERE airline ~ '[^a-z0-9_]';     -- 0
