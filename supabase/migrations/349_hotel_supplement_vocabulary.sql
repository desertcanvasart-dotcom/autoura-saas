-- ============================================================================
-- 349 — hotel supplements come from the tenant's vocabulary
-- ============================================================================
-- A hotel rate carries a list of supplements (accommodation_rates.supplements,
-- JSONB): a view, a floor, a meal plan, each with a per-night price by
-- season. The ten kinds on offer were hardcoded in the hotel rate form. They
-- are now a vocabulary kind, hotel_supplement; the entry's description
-- ("View", "Room", "Meal Plan") is the group the form's dropdown shows it
-- under, so no new settings field is needed.
--
-- The stored JSON already keyed each supplement by these same slugs
-- ({"type": "view_nile", "name": "Nile View", …}), so no row changes. The
-- `name` snapshot stays in the JSON; the form shows the vocabulary's word
-- when the key is known and the snapshot when it is not. The pricing
-- engine does not read this list.
--
-- seed_tenant_vocabulary is redefined in full (348's blocks verbatim plus
...
BEGIN
  FOR r IN SELECT id FROM tenants LOOP
    PERFORM seed_tenant_vocabulary(r.id, 'hotel_supplement');
  END LOOP;
END $$;
...
-- ----------------------------------------------------------------------------
-- Post-check:
--   SELECT count(*) FROM tenant_vocabularies WHERE kind = 'hotel_supplement';  -- 10 × tenants
