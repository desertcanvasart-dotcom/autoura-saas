...
export const VOCABULARY_KINDS = [
  ...
  'airport_direction', 'activity_pricing_type', 'guide_mode',
...
export const VOCABULARY_KIND_INFO: Record<VocabularyKind, VocabularyKindInfo> = {
  ...
  tipping_context: {
    ...
    description: 'The occasion a tip is for — it decides WHEN the tip is charged. Felucca, Motorboat and any context you add are not priced yet: a tour\'s days do not record them.',
  ...
  tipping_unit: {
    ...
    group: 'Guides & tipping',
    title: 'Tipping units',
    description: 'How a tip is counted. Pricing knows the five built-in units — Per Day, Per Service, Per Night, Per Cruise, Per Person; a unit you add yourself is shown on the form but cannot be priced.',
    usedIn: 'Tipping rates, pricing engine',
    minItems: 1,
  ...
  guide_mode: {
    ...
    group: 'Guides & tipping',
    title: 'Guide modes',
    description: 'How the guide is engaged: a Spot guide on each sightseeing day in that city, or one Throughout guide who travels the whole trip (fee every day, a bed each night, one vehicle seat). A guide rate says which mode it is for, and a quote is priced only from rates of its mode.',
    usedIn: 'Guide rates, the calculator, pricing engine',
    minItems: 1,
  ...
  rate_season: {
    ...
    group: 'General',
    title: 'Rate seasons',
    description: "The supplier's season a rate row or a hotel / cruise rate period is tagged with (a sleeper fare for peak season, a contract period called Christmas). A label only: pricing reads a period's dates, never its word; the uplift you charge is the Demand calendar under Settings → Seasonal Premiums.",
    usedIn: 'Hotel and cruise rate periods, attraction and sleeping-train rate rows; the periods CSV (Season column)',
    minItems: 1,
...
export const TABLE_VOCABULARY_COLUMNS: Record<string, Record<string, VocabularyKind>> = {
  ...
  // cruise it is the ship's cabin category. Without this the importer would
  // check "Deluxe" against the sleeper cabins and refuse it.
