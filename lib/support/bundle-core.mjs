...
export function bundleFindings(bundle) {
  ...
  for (const cron of bundle.crons) {
    if (cron.lastRun === null) {
      findings.push(`The "${cron.name}" job has never run here. Scheduled jobs are your scheduler's responsibility on a self-hosted install — see docs/SELF-HOSTING.md.`)
    }
  }
