...
//
// The support bundle — what a self-hosted customer can safely send us
...
export function bundleFindings(bundle) {
  ...
  if (neverRun.length === bundle.crons.length && bundle.crons.length > 0) {
    ...
    findings.push(`No scheduled job has ever run on this install. Set up your scheduler: see "Scheduled jobs" in docs/SELF-HOSTING.md. Until then exchange rates never refresh, and every historical conversion in the reports quietly falls back to today's rate.`)
  } else if (neverRun.length) {
    findings.push(`These jobs have never run here: ${neverRun.join(', ')}. Check your scheduler covers all of them — see docs/SELF-HOSTING.md.`)
  }
