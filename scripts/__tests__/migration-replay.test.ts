...
describe('migration replay from scratch', () => {
  it('every non-exempt migration applies cleanly to a fresh database', async () => {
    ...
    // Migration 373 dropped bookings.status_override: the sibling's column,
    // pasted into this database with no migration here behind it. 373 said a
    // port would "arrive with its own migration, in this repo" — 389 is that
    // (the booking status control records an override). So a fresh build
    // HAS it now, created by this repo.
    const override = await db.query(`
      SELECT count(*)::int AS n FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'bookings' AND column_name = 'status_override'`)
    expect((override.rows[0] as { n: number }).n, 'created by migration 389, this repo').toBe(1)
    // 373 still behaves where it runs: removes the column, and a second run is
    // harmless; 389 then puts it back, idempotently.
    const sql373 = readFileSync(path.join(MIGRATIONS_DIR, '373_drop_bookings_status_override.sql'), 'utf8')
    const sql389 = readFileSync(path.join(MIGRATIONS_DIR, '389_booking_status_override.sql'), 'utf8')
