// ============================================
// POST /api/ai/suggest-reply — user-initiated WhatsApp draft generation
// GET  /api/ai/suggest-reply?whatsapp_conversation_id=... — fetch existing
//       pending drafts (used by the panel on mount to surface pre-generated ones)
// ============================================
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await generateDraftReplies({
      supabase, tenantId: tenant_id, channel: 'whatsapp',
      whatsappConversationId: whatsapp_conversation_id,
...
// Fetch existing pending drafts for the latest inbound in a conversation.
// Returns an empty list if no inbox row exists yet or no pending drafts.
export async function GET(request: NextRequest) {
  ...
  const waConvId = request.nextUrl.searchParams.get('whatsapp_conversation_id')
  if (!waConvId) {
    return NextResponse.json({ success: false, error: 'whatsapp_conversation_id required' }, { status: 400 })
  }
  ...
  const { data: thread } = await supabase
    ...
    .eq('channel', 'whatsapp')
    .eq('whatsapp_conversation_id', waConvId)
    .maybeSingle()
  ...
  // Find latest inbox row for this thread
  ...
  // Only surface drafts if inbox is still "fresh" (not already responded)
  if (inbox.status === 'responded' || inbox.status === 'skipped') {
    return NextResponse.json({ success: true, drafts: [] })
  }
