...
export default function SleepingTrainRatesContent() {
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = await dialog.confirmDelete('Sleeping Train Rates',
    ...
    try {
      const results = await Promise.allSettled(
        ids.map(id => fetch(`/api/rates/sleeping-trains/${id}`, { method: 'DELETE' }))
      )
                ...
                <tr>
                  <th className="px-4 py-2 text-center w-10">
                    <input
                      ...
                      aria-label="Select all sleeping train rates"
                    />
                  </th>
